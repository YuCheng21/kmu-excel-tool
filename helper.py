...
class Helper:
    ...
    def flatten(self):
        result_difficulty = []
        result_severity = []
        for index, date in enumerate(self.df_normalization.loc[:, 3]):
            ...
            row = self.df_normalization.iloc[index]
    ...
    def auto_run(self, input_file, output_file):
        ...
        for key, value in self.input_excel.items():
            ...
            if self.df_normalization is None:
                difficulty, severity = pd.DataFrame([[]]), pd.DataFrame([[]])
            else:
...
if __name__ == '__main__':
    helper = Helper()
    helper.auto_run("./input/all.xls", f"./output/{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.xlsx")
